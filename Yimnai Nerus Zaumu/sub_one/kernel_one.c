...
//Create kernel module that will eventually create proc file
	...
	entry = proc_create("jiffies",0777,NULL,&my_fops);
	if(!entry){
		return -1;	
	}else{
...
MODULE_DESCRIPTION("This module creates a proc file ie proc/jiffies once it is loaded\nunto the kernel and deletes it when removed from the kernel");
